...
class HomeActivity final : public Activity {
  ...
  static int menuItemToIndex(HomeMenuItem item, bool hasOpdsUrl, bool hasDelivery) {
    int i = 0;
    ...
    if (hasOpdsUrl) ++i;
    if (item == HomeMenuItem::BOOK_DELIVERY) return hasDelivery ? i : 0;
    if (hasDelivery) ++i;
  ...
  static HomeMenuItem indexToMenuItem(int idx, bool hasOpdsUrl, bool hasDelivery) {
    int i = 0;
    if (idx == i++) return HomeMenuItem::FILE_BROWSER;
    if (idx == i++) return HomeMenuItem::LIBRARY;
    if (hasOpdsUrl && idx == i++) return HomeMenuItem::OPDS_BROWSER;
    if (hasDelivery && idx == i++) return HomeMenuItem::BOOK_DELIVERY;
